...
mode=$2
sites=$3
PYTHONPATH=. python MNIST_dadtorch.py -ddp True --node-rank $rank --dad-reduction $mode --num-nodes $sites --dist-url tcp://10.245.10.78:8998 --master-addr 10.245.10.78 --master-port 8998 -ph train --dist-backend nccl --batch_size 64 -log "net_logs/"$mode"_DADs"$sites