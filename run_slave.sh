...
cd examples
python MNIST_dadtorch.py -ddp True --node-rank $rank --dad-reduction $mode --num-nodes $sites --dist-url tcp://10.245.12.102:8998 --master-addr 10.245.12.102 --master-port 8998 -ph train --dist-backend $backend --batch_size 64 -log "net_logs/"$branch"/"$mode"-DADs"$sites