...
PYTHONPATH=. python examples/mnist_transformer.py -ddp True --node-rank $rank --dad-reduction $mode --num-nodes $sites --dist-url tcp://10.245.10.98:8998 --master-addr 10.245.10.98 --master-port 8998 -ph train --dist-backend gloo -log "net_logs/b512_WIKITEXT_"$mode"_DADs"$sites"b"$actual_batch"r5"
